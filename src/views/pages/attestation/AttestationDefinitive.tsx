...
const AttestationDefinitive = () => {
  ...
  const loadStudents = async (filters: Record<string, any> = {}) => {
    setLoading(true)
    try {
      const response = await attestationService.getEligibleForDefinitive(filters)
  ...
  const handleFilterChange = (name: string, option: any) => {
    ...
    if (newCohort  !== 'all') filters.cohort           = newCohort
  ...
  const handlePreview = async (student: EligibleStudent) => {
    ...
    try {
      const [attestationUrl, birthCertResponse] = await Promise.all([
        attestationService.generateDefinitive(student.student_pending_student_id),
  ...
  const handleGenerate = async (studentPendingStudentId: number) => {
    ...
    try {
      const url = await attestationService.generateDefinitive(studentPendingStudentId)
      const link = document.createElement('a')
      link.href = url
      link.download = 'attestation-definitive.pdf'
  ...
  const handleUpdateNames = async (lastName: string, firstNames: string) => {
    ...
    const url = await attestationService.generateDefinitive(previewStudent.student_pending_student_id)
  ...
  const handleGenerateBulk = async () => {
    ...
    try {
      const Swal = (await import('sweetalert2')).default
      const url  = await attestationService.generateMultipleDefinitive(selectedStudents)

      const link = document.createElement('a')
      link.href = url
      link.download = 'attestations-definitives.pdf'
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      ...
      <CCol xs={12}>
        <CCard className="mb-4">
          <CCardHeader className="d-flex justify-content-between align-items-center">
            <div>
              <strong>Attestation de Fin de Cycle</strong>
              <small className="text-muted ms-2">— Étudiants ayant validé la dernière année de leur cycle</small>
            </div>
            {students.length > 0 && (
              ...
              <CTable striped hover responsive>
                <CTableHead>
                  <CTableRow>
                    ...
                    <CTableHeaderCell>Niveau obtenu</CTableHeaderCell>
                  ...
                  {students.length === 0 ? (
                    <CTableRow>
                      <CTableDataCell colSpan={8} className="text-center py-4 text-muted">
                        Aucun étudiant éligible à l'attestation de fin de cycle
                      </CTableDataCell>
                    </CTableRow>
                  ) : (
